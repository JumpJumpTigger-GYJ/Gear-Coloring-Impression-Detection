...
D_YAML = r".\models\cFastGAN\Discriminator.yaml"
...
def train(root, ckpt_dir=None, restart=False):
    ...
    params_group = [
        {"params": G.parameters()},
        {"params": D.parameters()},
        {"params": sn.parameters()},
        {"params": snc.parameters()}
    ]
    ...
    for epoch in range(EPOCH_START if restart else 0, EPOCH):
        ...
        for batch_i, (img_real, label_real) in enumerate(train):
            ...
            with torch.no_grad():
                ...
                z0 = torch.cat([z0, onehot_real], dim=1)
                z1 = torch.cat([z1, onehot_real], dim=1)

                img_sm0, img_sm1 = G(z0), G(z1)

            t0_div, t1_div = sn(img_real), sn(img_div)
            t0_sm, t1_sm = sn(img_sm0), sn(img_sm1)

            loss_div = sn.contrastive_loss(t0_div, t1_div, similar=False, margin=MARGIN)
            loss_sm = sn.contrastive_loss(t0_sm, t1_sm, similar=True, margin=MARGIN)

            loss_sn = loss_div + loss_sm
            ...
            if batch_i % 20 == 0:
                print(f"SN: div:{loss_div:.2f}, sm:{loss_sm:.2f}")

            #snc
            t0_sm, t1_sm = t0_sm.detach(), t1_sm.detach()
            t0_div, t1_div = t0_div.detach(), t1_div.detach()
            ...
            #disriminator
            out_fake = D([img_sm0, onehot_real])
            ...
            out = D([img_fake, torch.cat([onehot_real] * 2, dim=0)])
            t0, t1 = sn(img_fake0), sn(img_fake1)
            logits = snc(t0, t1)

            loss_G_D = -torch.mean(out[0])
            loss_G_cls = D.loss_classifier(out[1], torch.cat([label_real] * 2, dim=0))
...
if __name__ == "__main__":
    seed_everything(0)
    train(root=r".\checkpoints\cGAN_P0", ckpt_dir=None, restart=False)
